...
async function newsSearchHandler(e) {
  ...
  if (resp.data.state == "loginrequired") {
    ...
    for (let i = 0; i < articles_search.length && i < imageDiv.length; i++) {
      ...
      const savearticle = $(`#savearticle${i+1}`) // Jinja's loop.index starts at 1 - here we start at 0
      savearticle.attr("data-article-title", article.title)
      savearticle.attr("data-article-url", article.url)

      const commentArticle = $(`#commentArticle${i+1}`)
      commentArticle.attr("data-article-url", article.url)
      commentArticle.attr("data-article-imgurl", article.urlToImage)
      commentArticle.attr("data-article-description", article.description)

      lastIndex = i;    

    }

    $(".likearticle").remove(); // redudant... 
    // $(".savearticle_cls").remove(); // for now because elements don't match due to DOM not being refreshed or something...
    // $(".commentArticle").remove(); // for now because elements don't match due to DOM not being refreshed or something...
